...
export async function POST(req: Request) {
  try {
    ...
    const chunks = Array.from(chunkMap.values())
      ...
      .slice(0, maxChunks); const styleSnippets = (styleData || []) as StyleRow[];
    ...
    const useFilteredChunks = filteredChunks.length > 0;
    const useFilteredStyleSnippets = filteredStyleSnippets.length > 0;
    const scopedChunks = useFilteredChunks ? filteredChunks : chunks;
    const scopedStyleSnippets = useFilteredStyleSnippets
    ...
    return NextResponse.json({
      ...
      retrievalDiagnostics: {
        ...
        filters: {
          ...
          filterFallbackUsed: anyFilterRequested && !anyFilterApplied,
