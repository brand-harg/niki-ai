...
- Keep retrieved lecture evidence inside a short **Lecture Connection** section near the end of the answer and in citations/source cards.
...
- Do not add a **Lecture Connection** section unless Lecture Mode is on and retrieval context exists.
`.trim()
    : `
...
- Use lecture evidence only in a short **Lecture Connection** section near the end of the answer and in citations/source cards.
...
function buildModeReminderSystemMessage({
  ...
  if (hasLectureContext) {
    return `Current response mode: Nemanja Mode with Lecture Mode. Make the style visibly lecture-aligned: board setup, lecture-supported terminology/order, and a brief Lecture Connection near the end. Keep the main explanation independent; use lecture evidence only in that Lecture Connection and the source cards.${longFormSuffix}`;
  }

  if (knowledgeBaseActive) {
    return `Current response mode: Nemanja Mode with Lecture Mode, but no lecture context was retrieved. Use Nemanja classroom style without inventing lecture details, and keep a short Lecture Connection that plainly says no lecture source was available for this answer.${longFormSuffix}`;
  }

  return `Current response mode: Nemanja Mode with Lecture Mode. Use Nemanja classroom style, keep the answer self-contained, and include a short Lecture Connection that plainly says no lecture source was attached for this answer.${longFormSuffix}`;
}
